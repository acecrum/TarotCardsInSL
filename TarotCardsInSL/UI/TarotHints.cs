...
public class TarotHints
{
    ...
    public static void ShowHeldCardHint(Player player, CustomCard card)
    {
        ...
        display.Show(HeldCardTag, new BasicElement(85, $"<align=right><b><size=24><color={card.GlowColor.ToHex()}>{card.Name} {Type}</color></size></b></align>"));
    }
    ...
    public static void ShowPickUpHint(Player player, CustomCard card, Config config)
    {
        ...
        var display = RueDisplay.Get(player);
        display.Show(PickupCardTag, new BasicElement(850, $"<b><color={card.GlowColor.ToHex()}>{card.Name}</color></b>\n{description}"), duration);
    }
    ...
    public static void ScpPickUpHint(Player player, Config config)
    {
        ...
        if (!scpPlayer) return;
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(200, "<b><size=30>As an <color=red>SCP</color> you can pick up and use <color=yellow>Tarot Cards.</color></size></b>"), 7f);
    }
    ...
    public static void Scp079Hint(Player player, Config config)
    {
        ...
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(200, "<b><size=30>As <color=#6c6cff>SCP-079</color> you cannot grab <color=yellow>Tarot cards</color> however,\n you can craft a <color=yellow>Tarot card</color> that assists you every 3 minutes."), 10f);
    ...
    {
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(200, $"<size=30><b>You are the Tarot Master,\n you are able to craft <color=yellow>Tarot Cards</color> for your <color={player.Role.GetRoleColor().ToHex()}>teammates</color> to use! \n You also recieve buffs to make you stronger.<b></size>"), 10f);
    }
    
    public static void RecieveHint(Player player, CustomCard card, Player master)
    {
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(250, $"You recieved <color={card.GlowColor.ToHex()}>{card.Name}</color> from <color={master.Role.GetRoleColor().ToHex()}>{master.DisplayName}</color>!"), 3f);
    }

    public static void GiveHint(Player player, CustomCard card, Player target)
    {
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(250, $"You gave <color={card.GlowColor.ToHex()}>{card.Name}</color> to <color={target.Role.GetRoleColor().ToHex()}>{target.DisplayName}</color>!"), 3f);
    }
    ...
    public static void MasterTimer(Player player, int seconds)
    {
        ...
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(125, $"<align=right><b><size=24>Next card in {timer}</size></b></align>"), 1f);
    }
    ...
    public static void InactiveMasterTimer(Player player, int seconds)
    {
        ...
        var timer = $"{minutes}:{remainingSeconds:00}";
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(125, $"<align=right><b><size=24>Next card in <color=#808080>{timer}</color></size></b></align>"), 1.1f);
    }
    
    public static void CardFailHint(Player player)
    {
        var display = RueDisplay.Get(player);
        display.Show(new BasicElement(165, "<align=right><b><size=24><color=red>Nope!</color></size></b></align>"), 3f);
    }
